...
ConsoleLibrary::ConsoleLibrary() {
	...
	SetTextColor(ConsoleLibrary::Color::Red | ConsoleLibrary::Color::Green | ConsoleLibrary::Color::Blue);
...
void ConsoleLibrary::Clear(void) {
	...
	FillConsoleOutputAttribute(m_Screen, m_TextColor | m_BackgroundColor, 80 * 60, Start, &Written);
	FillConsoleOutputCharacterA(m_Screen, ' ', 80 * 60, Start, &Written);
...
void ConsoleLibrary::OutputString(const std::string &String) const{
	DWORD Written;

	WriteConsoleA(m_Screen, String.c_str(), String.size(), &Written, NULL);
}
